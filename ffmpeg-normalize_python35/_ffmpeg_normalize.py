...
def check_range(number, min_r, max_r, name=""):
    ...
    try:
        ...
        if number < min_r or number > max_r:
            raise FFmpegNormalizeError(name+" must be within ["+min_r+","+max_r+"]")
        return number
        pass
...
class FFmpegNormalize:
    ...
    def __init__(
        ...
        if (
            ...
            raise FFmpegNormalizeError(
                "Output format "+self.output_format+" does not support PCM audio. "
                + "Please choose a suitable audio codec with the -c:a option."
            )

        if normalization_type not in NORMALIZATION_TYPES:
            raise FFmpegNormalizeError(
                "Normalization type must be one of "+NORMALIZATION_TYPES
            )
    ...
    def add_media_file(self, input_file, output_file):
        ...
        if (
            ...
            raise FFmpegNormalizeError(
                "Output extension "+ext+" does not support PCM audio. " +
                "Please choose a suitable audio codec with the -c:a option."
            )
    ...
    def run_normalization(self):
        ...
        for index, media_file in enumerate(
            ...
            logger.info(
                "Normalizing file "+str(media_file)+" ("+str(index+1)+" of "+str(self.file_count)+")"
            )
            ...
            logger.info("Normalized file written to "+media_file.output_file)
