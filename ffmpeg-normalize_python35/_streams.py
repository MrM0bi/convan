...
class AudioStream(MediaStream):
    ...
    def get_pcm_codec(self):
        if not self.bit_depth:
            ...
            return "pcm_s"+self.bit_depth+"le"
        else:
            logger.warning(
                "Unsupported bit depth "+self.bit_depth+", falling back to pcm_s16le"
            )
            return "pcm_s16le"
    ...
    def _get_filter_str_with_pre_filter(self, current_filter):
        ...
        """
        input_label = "[0:"+str(self.stream_id)+"]"
        filter_chain = []
    ...
    def parse_volumedetect_stats(self):
        ...
        """
        logger.info(
            "Running first pass volumedetect filter for stream "+self.stream_id
        )
        ...
        if mean_volume_matches:
            ...
            raise FFmpegNormalizeError(
                "Could not get mean volume for "+self.media_file.input_file
            )
        ...
        if max_volume_matches:
            ...
            raise FFmpegNormalizeError(
                "Could not get max volume for "+self.media_file.input_file
            )
    ...
    def parse_loudnorm_stats(self):
        ...
        """
        logger.info("Running first pass loudnorm filter for stream "+str(self.stream_id))
    ...
    def _parse_loudnorm_output(output_lines):
        ...
        try:
            ...
            logger.debug("Loudnorm stats parsed: "+json.dumps(loudnorm_stats))
