...
class MediaFile:
    ...
    def parse_streams(self):
        ...
        """
        logger.debug("Parsing streams of "+self.input_file)
        ...
        for line in output_lines:
            ...
            if "Audio" in line:
                logger.debug("Found audio stream at index "+str(stream_id))
                ...
                logger.debug("Found video stream at index "+stream_id)
                self.streams["video"][stream_id] = VideoStream(
                    self, self.ffmpeg_normalize, stream_id
                )

            elif "Subtitle" in line:
                logger.debug("Found subtitle stream at index "+stream_id)
        ...
        if not self.streams["audio"]:
            raise FFmpegNormalizeError(
                "Input file "+self.input_file+" does not contain any audio streams"
            )
    ...
    def run_normalization(self):
        logger.debug("Running normalization for "+self.input_file)
    ...
    def _first_pass(self):
        logger.debug("Parsing normalization info for "+self.input_file)
        ...
        for index, audio_stream in enumerate(self.streams["audio"].values()):
            ...
            if self.ffmpeg_normalize.progress:
                with tqdm(
                    total=100,
                    position=1,
                    desc="Stream "+index + 1+"/"+len(self.streams['audio'].values()),
                ) as pbar:
                    for progress in fun():
                        pbar.update(progress - pbar.n)
            else:
    ...
    def _get_audio_filter_cmd(self):
        ...
        for audio_stream in self.streams["audio"].values():
            ...
            input_label = "[0:"+str(audio_stream.stream_id)+"]"
            output_label = "[norm"+str(audio_stream.stream_id)+"]"
    ...
    def _second_pass(self):
        ...
        """
        logger.info("Running second pass for "+self.input_file)
        ...
        if self.ffmpeg_normalize.metadata_disable:
            ...
            for stream_type in output_stream_types:
                ...
                for idx, _ in enumerate(self.streams[stream_type].items()):
                    cmd.extend(
                        [
                            "-map_metadata:s:"+str(stream_key)+":"+str(idx),
                            "0:s:"+str(stream_key)+":"+str(idx),
                        ]
                    )
        ...
        if self.ffmpeg_normalize.audio_codec:
            ...
            for index, (_, audio_stream) in enumerate(self.streams["audio"].items()):
                cmd.extend(["-c:a:"+index, audio_stream.get_pcm_codec()])
        ...
        if self.ffmpeg_normalize.keep_original_audio:
            ...
            for index, (_, s) in enumerate(self.streams["audio"].items()):
                cmd.extend(["-map", "0:a:"+index])
                cmd.extend(["-c:a:"+highest_index + index, "copy"])
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    "Moving temporary file from "+temp_file_name+" to "+self.output_file
                )
