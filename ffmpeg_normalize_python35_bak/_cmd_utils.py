...
class CommandRunner:
    ...
    def run_command(self):
        logger.debug("Running command: "+str(self.cmd))
        ...
        if p.returncode == 0:
            ...
            raise RuntimeError(
                "Error running command "+self.cmd+": "+str(stderr)
            )
...
def dict_to_filter_opts(opts):
    ...
    for k, v in opts.items():
        filter_opts.append(str(k)+"="+str(v))
    ...
    if ffmpeg_path:
        if os.sep in ffmpeg_path:
            ...
            if not os.path.isfile(ffmpeg_exe):
                raise FFmpegNormalizeError("No file exists at "+ffmpeg_exe)
        else:
            ffmpeg_exe = which(ffmpeg_path)
            if not ffmpeg_exe:
                raise FFmpegNormalizeError(
                    "Could not find '"+ffmpeg_path+"' in your $PATH."
                )
    else:
